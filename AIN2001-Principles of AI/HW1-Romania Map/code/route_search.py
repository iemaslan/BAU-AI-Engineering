...
def draw_map(problem,ax):
    ...
    for link in distances_dict:
        ...
        ax.plot([b_coordinates[0],e_coordinates[0]], [b_coordinates[1],e_coordinates[1]], marker='o', markersize=5, color='k')
        ax.text((b_coordinates[0]+e_coordinates[0])*0.5*0.98, (b_coordinates[1]+e_coordinates[1])*0.5*0.98, distance, horizontalalignment='left', verticalalignment='top')
    ...
    for city in city_coordinates_dict:
        ...
        ax.text(city_coordinates[0]*0.98, city_coordinates[1]*0.98, '{}({})'.format(city,int(heuristic_value)), horizontalalignment='right', verticalalignment='top', bbox={'facecolor':'gray', 'alpha':0.5})
...
parser.add_argument('--search_algorithm', type=str, default='depth_first_graph_search', help='search algorithm')
